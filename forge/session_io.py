...
class SessionManager:
    """Manages session lifecycle, transcript I/O, state, and operator intervention."""
    ...
    @classmethod
    def resume(cls, work_dir: Path, agents: list[Agent]) -> tuple["SessionManager", dict]:
        """Load existing session from state.json, rebuild history if needed.
        Returns (SessionManager, state_dict).
        """
        transcript = work_dir / "transcript.md"
        state_file = work_dir / "state.json"
        orch_log = work_dir / "orchestrator.log"
        ...
        return cls(session), state

    # -------------------------------------------------------------------
    # State management
    # -------------------------------------------------------------------

    def update_state(self, status: str, agents: list[Agent],
                     max_turns: int, model: str, mission_source: str) -> None:
        """Save session state to state.json."""
        state = {
            "utterances": self._session.utterances,
            "max_turns": max_turns,
            "status": status,
            "last_speaker": self._session.last_speaker,
            "consecutive_count": self._session.consecutive_count,
            "speakers_history": self._session.speakers_history,
            "agents": [a.name for a in agents],
            "model": model,
            "mission_source": mission_source,
            "interventions": self._session.interventions,
        }
        self._session.state_file.write_text(json.dumps(state, indent=2), encoding="utf-8")

    def archive_outputs(self) -> str:
        ...
        for fname in ("closing.md", "synthesis.md"):
            fpath = self._session.work_dir / fname
            if fpath.exists():
                stem = fpath.stem
    ...
    def append_agent_turn(self, speaker: str, response: str,
        ...
        (self._session.utterances_dir / f"{ts}-{safe_name}.md").write_text(
            f"# {speaker} -- Turn {self._session.utterances + 1}"
        ...
        with self._session.transcript.open("a", encoding="utf-8") as f:
            f.write(f"### Turn {self._session.utterances + 1} -- {speaker} [{turn_time}]"
                    + (f" [execute]" if action == "execute" else "") + "\n"
                    + (f"**Task**: {task_str}\n\n" if action == "execute" else "")
                    + f"{response}\n\n---\n\n")

    def append_orchestrator_turn(self, pick_result: dict) -> None:
        ...
        (self._session.utterances_dir / f"{ts}-host.md").write_text(
            f"# Orchestrator -- Turn {self._session.utterances + 1}\n\n"
    ...
    def append_verification(self, turn_label: str, status: str,
                            details: str, attempt_label: str = "") -> None:
        """Write verification to transcript + notes/_verification/."""
        verify_notes = self._session.notes_dir / "_verification"
    ...
    def get_transcript_context(self, recent_turns: int = 7) -> str:
        content = self._session.transcript.read_text(encoding="utf-8")
        if self._session.utterances <= recent_turns:
            return content
        ...
        omitted = self._session.utterances - recent_turns
        return (f"{header}\n\n"
                f"[Turns 1-{omitted} omitted -- key findings are in notes/ folders]"
                f"\n\n{''.join(recent)}")

    def truncate_transcript_for_closing(self, keep_recent: int = 15) -> str:
    ...
    def inject_operator_turn(self, msg: str) -> None:
        """Inject an operator message as a turn in the transcript."""
        turn_num = self._session.utterances + 1
        turn_time = datetime.now().strftime("%H:%M")
        ts = datetime.now().strftime("%Y%m%d-%H%M%S")

        (self._session.utterances_dir / f"{ts}-operator.md").write_text(
        ...
        op_notes = self._session.notes_dir / "_operator"
        ...
        self._session.utterances += 1
        self._session.last_speaker = "[operator]"
        self._session.speakers_history.append("[operator]")
        self._session.interventions.append({
            "turn": turn_num,
            "type": "inject",
