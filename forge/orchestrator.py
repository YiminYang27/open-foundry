...
def orchestrator_pick(session: Session, ctx: ForumContext,
                      llm: ClaudeCLI) -> dict:
...
def verify_task(orch: Orchestrator, task: dict,
...
def finalize(session: Session, ctx: ForumContext, llm: ClaudeCLI,
...
def execution_phase(session: Session, ctx: ForumContext,
    ...
    # Import here to avoid circular import (agents imports from this module's siblings)
    from forge.agents import agent_execute
    ...
    for idx, task_def in enumerate(tasks):
        ...
        for retry in range(max_task_retries):
            verification = verify_task(ctx.orch, task_def, response, llm)
